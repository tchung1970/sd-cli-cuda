...
in_log_cb && /^\}/ {
    ...
    print "            LOG_WARN(\"CUDA out of memory!\");"
    print "            LOG_WARN(\"  Device %d (%s) has only %zu MB free, need %zu MB\", i, description, free_mb, min_required_mb);"
    print "            LOG_WARN(\"  Close GPU apps (ComfyUI, etc): pkill -f comfyui\");"
    print "            exit(1);"
    print "        }"
    print "    }"
    print "    return true;"
