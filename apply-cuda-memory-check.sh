...
in_log_cb && /^\}/ {
    ...
    print "            LOG_WARN(\"WARNING: CUDA device %d has only %zu MB free memory!\", i, free_mb);"
    print "            LOG_WARN(\"  Stable Diffusion typically requires at least %zu MB of GPU memory.\", min_required_mb);"
    print "            LOG_WARN(\"  Please close GPU-intensive applications to free up memory:\");"
    print "            LOG_WARN(\"    - ComfyUI, Other AI/ML applications, Video editors\");"
    print "            LOG_WARN(\"  To kill ComfyUI: pkill -f comfyui || pkill -f ComfyUI\");"
    print "            LOG_WARN(\"  Continuing anyway, but you may encounter out-of-memory errors...\");"
    print "            return false;"
    print "        }"
    print "    }"
    print "    return true;"
